...
/// <summary>
/// This class is a bunch of static methods that are used to calculate the map cells.
/// This should even work if the map system is changed.
/// </summary>
public static class MapTools
{
    ...
    public static readonly int[] EveryCellId;
    ...
    static MapTools()
    {
        ...
        EveryCellId  = new int[MapCountCell];
    ...
    public static int GetLookDirection4(int cellId1, int cellId2)
    {
        ...
        return GetLookDirection4ByCoord(cellCoord1.Value.X, cellCoord1.Value.Y, cellCoord2.Value.X, cellCoord2.Value.Y);
    }
    ...
    /// <summary>
    /// Returns a value representing the diagonal direction between two coordinates.
    /// </summary>
    /// <param name="xCoord1">The X coordinate of the first cell.</param>
    /// <param name="yCoord1">The Y coordinate of the first cell.</param>
    /// <param name="xCoord2">The X coordinate of the second cell.</param>
    ...
    public static int GetNextCellByDirectionAndCoord(int xCoord, int yCoord, int direction)
    {
        ...
        return GetCellIdByCoord(xCoord + CoordinatesDirection[direction].X, yCoord + CoordinatesDirection[direction].Y);
    }
    ...
    /// <summary>
    /// Determines if adjacent cells allow access in the given fight context, starting cell, and direction.
    /// </summary>
    /// <param name="fightContext">The fight context in which the access is being checked.</param>
    /// <param name="startCell">The starting cell of the path.</param>
    /// <param name="direction">The direction of the path.</param>
    /// <returns>True if adjacent cells allow access, false otherwise.</returns>
    public static bool AdjacentCellsAllowAccess(FightContext fightContext, int startCell, int direction)
    {
        var nextCellClockwise        = GetNextCellByDirection(startCell, (direction + 1) % 8);
        var nextCellCounterClockwise = GetNextCellByDirection(startCell, (direction + 8 - 1) % 8);

        if (MapDirection.IsOrthogonal(direction) || fightContext.IsCellEmptyForMovement(nextCellClockwise) &&
            fightContext.IsCellEmptyForMovement(nextCellCounterClockwise))
        {
            return true;
        }
