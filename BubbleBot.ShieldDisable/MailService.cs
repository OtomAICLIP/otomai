...
public class MailService : Singleton<MailService>
{
    ...
    public async Task<(string, string)> ReorderMail(string mail)
    {
        var apiKey = GetApiKey();
        using var httpClient = new HttpClient();
        var response = await httpClient.GetStringAsync($"https://api.kopeechka.store/mailbox-reorder?site=ankama.com&email={mail}&token={apiKey}&subject=Votre%20code%20de%20s%C3%A9curit%C3%A9%20Ankama&api=2.0");
    ...
    public async Task<string> ReceiveMail(string mailId)
    {
        var tries = 0;
        var title = "WAIT_LINK";
