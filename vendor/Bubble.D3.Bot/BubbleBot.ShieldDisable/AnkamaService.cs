...
public class AnkamaService
{
    ...
    public async Task<bool> ConnectAnkama(string login, string password, AwsBypassService awsBypass)
    {
        ...
        if(loginCode == "cloudfont" || string.IsNullOrEmpty(loginCode))
        {
            Console.WriteLine("Cloudfront error");
            return false;
        }
    ...
    public async Task<bool> DisableShield(string code)
    {
        ...
        if (response.Contains("Installez la sécurité par email"))
        {
            return true;
        }
        else
    ...
    private async Task<(string state, string codeChallenge)> GetState2(string url)
    {
        ...
        var response2 = await _client.GetAsync(
            $"https://auth.ankama.com/login/ankama?direct=&origin_tracker=https://www.ankama.com/en&code_challenge={codeChallenge}&redirect_uri=https://account.ankama.com/authorized&client_id=0");
