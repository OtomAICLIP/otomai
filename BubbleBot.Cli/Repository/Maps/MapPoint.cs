...
{
    public const int CapabilityAllowChallenge = 1;
    public const int CapabilityAllowAggression = 2;
    public const int CapabilityAllowTeleportTo = 4;
    public const int CapabilityAllowTeleportFrom = 8;
    public const int CapabilityAllowExchangesBetweenPlayers = 16;
    public const int CapabilityAllowHumanVendor = 32;
    public const int CapabilityAllowCollector = 64;
    public const int CapabilityAllowSoulCapture = 128;
    public const int CapabilityAllowSoulSummon = 0x0100;
    public const int CapabilityAllowTavernRegen = 0x0200;
    public const int CapabilityAllowTombMode = 0x0400;
    public const int CapabilityAllowFightChallenges = 0x1000;
    public const int CapabilityAllowTeleportEverywhere = 0x0800;
    public const int CapabilityAllowMonsterRespawn = 0x2000;
    public const int CapabilityAllowMonsterFight = 0x4000;
    public const int CapabilityAllowMount = 0x8000;
    public const int CapabilityAllowObjectDisposal = 0x10000;
    public const int CapabilityAllowUnderwater = 0x20000;
    public const int CapabilityAllowPvp1V1 = 0x40000;
    public const int CapabilityAllowPvp3V3 = 0x80000;
...
public sealed class MapPoint
{
    ...
    public static readonly Dictionary<int, MapPoint> OrthogonalGridReferenceDic = new Dictionary<int, MapPoint>();
    ...
    static MapPoint()
    {
        ...
        for (short i = 0; i < OrthogonalGridReference.Length; i++)
        {
            var cellId = OrthogonalGridReference[i].CellId;
            OrthogonalGridReferenceDic[cellId] = OrthogonalGridReference[i];
        }
    }
    ...
    public Direction OrientationTo(MapPoint point, bool diagonal = true)
    {
        ...
        var orientation = !diagonal
            ? MathUtils.Round(transformedAngle / 90) * 2 + 1
            : MathUtils.Round(transformedAngle / 45) + 1;
    ...
    public Direction OrientationToX(MapPoint point, bool diagonal = false)
    {
        ...
        var orientation = !diagonal
            ? MathUtils.Round(angleInDegrees / 90) * 2 + 1
            : MathUtils.Round(angleInDegrees / 45) + 1;
    ...
    public IEnumerable<MapPoint> GetAdjacentCells(Func<int, bool> predicate, bool diagonal = false, byte range = 1)
    {
    ...
    public uint AdvancedOrientationTo(MapPoint? target, bool fourDir = true)
    {
        ...
        var angle =
            Math.Acos(xDifference / Math.Sqrt(Math.Pow(xDifference, 2) + Math.Pow(yDifference, 2))) *
            180 /
            Math.PI *
            (target.Y > Y ? -1 : 1);
        ...
        if (fourDir)
        {
            angle = MathUtils.Round(angle / 90) * 2 + 1;
        }
        else
        {
            angle = MathUtils.Round(angle / 45) + 1;
        }
